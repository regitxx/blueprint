...
> Rebuild Blueprint in AI Studio by pasting each numbered prompt below, in order, into the
> Build agent. Prompt 1 bootstraps the app; prompts 2–7 apply the exact reference files.
> After the last prompt, run the checklist at the bottom.
...
Set up the app shell and data model for Blueprint: page frame, blueprint design system, and the shared TypeScript contracts every agent uses.

Apply these reference implementations exactly — create or overwrite each file:
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .export-row { display: flex; flex-wrap: wrap; gap: 6px; align-items: center; margin-top: 12px; }
  .export-row .chip-btn { font-family: var(--mono); font-size: 11.5px; padding: 3px 10px; }
...
Add the agent system prompts and two real saved pipeline runs used as instant example blueprints.
...
Implement live source gathering: a 4-rail arXiv fallback ladder, GitHub search with README enrichment, and a GitHub repo skeleton fetcher for as-built mapping.
...
Implement the Gemini brain: seven schema-contracted agent calls (interpreter, reader, cartographer, scout, analyst, architect, refine) with retry, thinking auto-degrade, and server-proxy key handling.
...
Implement the results view: variant sheets with engineering title blocks, Mermaid rendering with fallback, trade-off table, cited sources.
...
Wire the full product: pipeline orchestration, ambiguity cards, doc upload (.txt/.md/.docx), run history, conversational refine, and ADR/agent-prompt exports.
...
- [ ] Delete any leftover skeleton files not in the list above.
- [ ] Secrets → set `GEMINI_API_KEY` = the paid key.
- [ ] Preview one run.
- [ ] Publish.
- [ ] 3-tab burst test (open 3 tabs, run concurrently).
